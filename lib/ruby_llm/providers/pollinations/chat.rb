...
module RubyLLM
  module Providers
    class Pollinations
      # Chat methods of the Pollinations API integration
      module Chat
        def completion_url
          'v1/chat/completions'
        end
        ...
        def render_payload(messages, tools:, temperature:, model:, stream: false, schema: nil, thinking: nil) # rubocop:disable Metrics/ParameterLists
          ...
          payload[:tools] = tools.map { |_, tool| Pollinations::Tools.tool_for(tool) } if tools.any?
        ...
        def parse_completion_response(response)
          ...
          Message.new(
            role: :assistant,
            content: content,
            thinking: Thinking.build(text: thinking_text),
            tool_calls: Pollinations::Tools.parse_tool_calls(message_data['tool_calls']),
        ...
        def format_messages(messages)
          messages.map do |msg|
            {
              role: msg.role.to_s,
              content: Pollinations::Media.format_content(msg.content),
              tool_calls: Pollinations::Tools.format_tool_calls(msg.tool_calls),
